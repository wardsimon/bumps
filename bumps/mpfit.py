   ...
   .perror
      ...
      *If* you can assume that the true reduced chi-squared value is unity --
      ...
      """
        ...
        xall = numpy.asarray(xall, np_float)
        ...
        if ((self.status > 0) and (nocovar == 0) and (n is not None)
            ...
            if ((n > 0) and (sz[0] >= n) and (sz[1] >= n)
                ...
                ## parameters.
                self.covar = numpy.zeros([nn, nn], np_float)
                for i in range(n):
                ...
                self.perror = numpy.zeros(nn, np_float)
    ...
    def parinfo(self, parinfo=None, key='a', default=None, n=0):
        ...
        if isinstance(test, int):
            values = numpy.asarray(values, np_int)
        elif isinstance(test, float):
            values = numpy.asarray(values, np_float)
        return (values)
    ...
    def fdjac2(self, fcn, x, fvec, step=None, ulimited=None, ulimit=None, dside=None,
        ...
        if (autoderivative == 0):
            mperr = 0
            fjac = numpy.zeros(nall, np_float)
    ...
    def qrsolv(self, r, ipvt, diag, qtb, sdiag):
        ...
        # if (self.debug) print("wa = ",wa, ipvt)
        numpy.put(x, ipvt, wa)
        return (r, x, sdiag)
